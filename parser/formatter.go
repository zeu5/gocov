...
func NewFormatter(cm CounterMode) *Formatter {
	return &Formatter{
...
func (fm *Formatter) AddUnit(file string, fname string, isfnlit bool, unit CoverableUnit, count uint32) {
	if fm.p == nil {
	...
	if fm.cm == CtrModeSet {
		...
		result, _ = SaturatingAdd(pcount, count)
	}
